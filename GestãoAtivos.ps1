...
function Show-Menu {
    ...
    Write-Host "7. Limpar equipamentos inativos (90 dias)" -ForegroundColor Green
...
function Get-Inventory {
    ...
    try {
        ...
        $computers = Get-ADComputer -Filter {Enabled -eq $true} -SearchBase $BaseOU -Properties Name, OperatingSystem, LastLogonDate, DistinguishedName -Server $Domain -ErrorAction Stop
        Write-Host "Buscando usuários na OU $BaseOU..." -ForegroundColor Yellow
        $users = Get-ADUser -Filter {Enabled -eq $true} -SearchBase $BaseOU -Properties SamAccountName, Enabled, Department, LastLogonDate, DistinguishedName -Server $Domain -ErrorAction Stop
        ...
        foreach ($computer in $computers) {
            ...
            if (Test-Connection -ComputerName $computer.Name -Count 1 -Quiet -ErrorAction SilentlyContinue) {
                ...
                # Obter fabricante e modelo via WMI, se disponível
                try {
                    ...
                    Write-Log "Erro ao obter informações de hardware para $($computer.Name): $_"
                }
            }
            ...
            } catch { Write-Log "Erro ao obter usuário do computador $($computer.Name): $_" }
...
function List-ByDepartment {
    ...
    foreach ($dept in $departments) {
        ...
        Write-Host "----------------------------------------"
        $dept.Group | Format-Table ComputerName, User, OperatingSystem, LastLogonDate, OnlineStatus -AutoSize
    }
    Pause
}
...
function List-ByUser {
    ...
    foreach ($user in $users) {
        ...
        Write-Host "----------------------------------------"
        $user.Group | Format-Table ComputerName, Department, OperatingSystem, LastLogonDate, OnlineStatus -AutoSize
    }
    Pause
}
...
function List-InactiveUsers {
    ...
    if ($inactive) {
        ...
        Write-Host "----------------------------------------"
        $inactive | Format-Table ComputerName, User, Department, OperatingSystem, LastLogonDate, OnlineStatus -AutoSize
    } else {
...
function List-OnlineOffline {
    ...
    Write-Host "----------------------------------------"
    $inventory | Where-Object { $_.OnlineStatus -eq "Online" } | Format-Table ComputerName, IPAddress, User, Department, OperatingSystem -AutoSize
    Write-Host "`nEquipamentos Offline" -ForegroundColor Cyan
    Write-Host "----------------------------------------"
    $inventory | Where-Object { $_.OnlineStatus -eq "Offline" } | Format-Table ComputerName, User, Department, OperatingSystem -AutoSize
    Pause
}
...
function List-OrphanedEquipment {
    ...
    if ($orphaned) {
        ...
        Write-Host "----------------------------------------"
        $orphaned | Format-Table ComputerName, Department, OperatingSystem, LastLogonDate, OnlineStatus -AutoSize
    } else {
...
function Export-Report {
    ...
    Write-Log "Relatório exportado para $filePath"
...
function Clear-InactiveEquipment {
    ...
    if ($inactive) {
        ...
        Write-Host "----------------------------------------"
        $inactive | Format-Table ComputerName, LastLogonDate, User, Department -AutoSize
...
do {
    ...
    switch ($choice) {
        ...
        "6" { Export-Report }
        "7" { Clear-InactiveEquipment }
        "8" { Write-Host "Sair..." -ForegroundColor Red; break }
